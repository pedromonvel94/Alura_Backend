...
public class PrincipalConBusqueda {
    ...
    HttpResponse<String> response = client //Con el HttpResponse indicamos que es lo que queremos recibir del servidor, en este caso estamos indicando que queremos recibir un String como respuesta del servidor
            .send(request, HttpResponse.BodyHandlers.ofString()); //Como podemos ver está enviando nuestra request (lo que nosotros queremos pedir) y ademas le estamos diciendo que queremos recibir la respuesta como un String usando el BodyHandler.ofString() (básicamente el BodyHandler lo que hace es interpretar lo que nosotros queremos recibir).

    System.out.println(response.body()); //Con este metodo body() lo que hacemos es obtener el cuerpo de la respuesta que nos envio el servidor (en este caso la informacion de la pelicula Matrix en formato JSON)
    }
